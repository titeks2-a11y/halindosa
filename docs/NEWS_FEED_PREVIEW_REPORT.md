...
- 생성 시각: 2026-06-12T22:20:59.663Z
...
- `/admin`의 `공식 뉴스·혜택 feed 붙여넣기 검증` 패널은 운영자가 받은 JSON/RSS/CSV/NDJSON 본문을 임의 URL 서버 fetch 없이 POST `/api/admin/news-feed-preview`로 검증합니다.
- 검색 결과, 커뮤니티 원문, 뉴스 기사 단독 링크는 사용자 노출에서 제외됩니다.
- RSS 본문 안 공식 이벤트 링크가 있으면 기사 링크는 sourceUrl로 남고 공식 링크가 finalUrl로 승격됩니다.
- CSV/NDJSON feed도 같은 공식 링크, 중복 후보, 종료 문구, 만료일 검사를 거친 뒤 visibleRows만 운영 후보가 됩니다.
