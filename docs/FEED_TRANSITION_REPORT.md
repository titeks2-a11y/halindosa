...
- 생성 시각: 2026-06-12T15:19:50.338Z
...
| event_news | 이벤트 뉴스 | seed fallback | 12 | 0 | seed 운영 | DEAL_EVENT_NEWS_FEED_URLS | 뉴스 원문은 sourceUrl로만 보관하고 사용자 이동 finalUrl은 공식 이벤트 상세로 교체 |
| official_event | 공식 이벤트·혜택 | seed fallback | 130 | 0 | seed 운영 | OFFICIAL_EVENT_FEED_URLS, DEAL_EVENT_FEED_URLS | 상용 운영 전 가장 먼저 공식 이벤트 feed를 연결해 seed 의존도 축소 |
| public_coupon | 공공·쿠폰·무료혜택 | seed fallback | 51 | 0 | seed 운영 | PUBLIC_COUPON_FEED_URLS, BENEFIT_REFRESH_FEED_URLS | 무료·쿠폰·포인트 혜택은 공식 수령 페이지가 있는 feed만 연결 |
...
4. `BENEFIT_REFRESH_FEED_URLS`
5. `DEAL_NEWS_FEED_URLS`
6. `DEAL_NEWS_RSS_URLS`
7. `DEAL_EVENT_NEWS_FEED_URLS`
