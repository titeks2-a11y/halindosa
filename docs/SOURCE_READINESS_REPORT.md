...
- 생성 시각: 2026-06-13T00:53:18.277Z
- 준비 상태: seed launch ready / 공식 feed 연결 대기
- 출시 게이트: passed
- 공식 소스 후보: 230개
- 접근 가능/보호 소스: 203개 / 27개
- 설정된 공식 feed URL: 0개
- 공식 혜택 노출 가능: 197개
- 차단 이슈: 0개
- 소비자형 공식 혜택 소스: 182개 (79%)
...
| 게이트 | 상태 | 내용 | 다음 작업 |
| --- | --- | --- | --- |
| official source catalog | passed | 230개 공식 소스 후보, 누락 카테고리 0개, 얇은 카테고리 0개 | npm run source:catalog:report |
| official source live | passed | 접근 가능 203개, 보호 27개, 차단 이슈 0개, 네트워크 점검 0개 | npm run source:live:doctor |
| source onboarding plan | passed | env 후보 11개, 상위 액션 10개 | npm run source:onboarding:plan |
...
| consumer first source mix | passed | 소비자형 79%, 공공/정책성 21%, high priority 소비자형 62개 | npm run source:breadth:doctor |

## 공식 feed env 연결 후보

| Env key | 상태 | 후보 | 접근 가능 | 보호/승인 | 설정 URL | 다음 작업 |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| OFFICIAL_EVENT_FEED_URLS | ready_to_connect | 196 | 174 | 22 | 0 | OFFICIAL_EVENT_FEED_URLS에 공식 JSON/RSS 또는 승인된 파트너 feed URL을 줄바꿈 또는 쉼표로 입력 |
| BENEFIT_REFRESH_FEED_URLS | ready_to_connect | 138 | 124 | 14 | 0 | BENEFIT_REFRESH_FEED_URLS에 공식 JSON/RSS 또는 승인된 파트너 feed URL을 줄바꿈 또는 쉼표로 입력 |
| PUBLIC_COUPON_FEED_URLS | ready_to_connect | 134 | 117 | 17 | 0 | PUBLIC_COUPON_FEED_URLS에 공식 JSON/RSS 또는 승인된 파트너 feed URL을 줄바꿈 또는 쉼표로 입력 |
...
| PAY_POINT_BENEFIT_FEED_URLS | ready_to_connect | 9 | 9 | 0 | 0 | PAY_POINT_BENEFIT_FEED_URLS에 공식 JSON/RSS 또는 승인된 파트너 feed URL을 줄바꿈 또는 쉼표로 입력 |
...
| SIGNUP_GIFT_FEED_URLS | ready_to_connect | 3 | 3 | 0 | 0 | SIGNUP_GIFT_FEED_URLS에 공식 JSON/RSS 또는 승인된 파트너 feed URL을 줄바꿈 또는 쉼표로 입력 |
