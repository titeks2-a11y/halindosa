...
- 최신 운영 확인 기준: `6eb18829` 기준 Vercel Production 반영 확인. 최신 코드 커밋은 `84336264`이며 Vercel Hobby 일일 배포 제한 해제 후 운영 반영을 재시도한다.
...
- 2026-06-13 05:00 KST 확인: 최신 커밋 `84336264`는 즉시수령 무료혜택 메타와 홈 노출을 강화했고, `npm run lint`, `npm run smoke:local`, `npm run release:doctor`, `npm run build`, `npm run build:android`, `npm run cap:sync`, `npm run android:webview:doctor`, `npm run workspace:doctor:strict`를 통과했다. 직접 `vercel deploy --prod --force --yes`는 Hobby 일일 배포 제한(`api-deployments-free-per-day`)이 남아 있으면 실패할 수 있다. 운영 `/api/health.deployment.shortCommit`은 아직 `6eb18829`이다. 제한 해제 또는 GitHub Actions 재실행 후 최신 커밋을 배포하고 `/api/health.deployment.shortCommit=84336264`를 확인한다.
