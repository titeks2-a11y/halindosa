...
- 2026-06-13 KST 추가 확인: `deploy:promote:latest` 스크립트를 추가했다. 이 명령은 최신 `Ready` Preview를 자동 선택하고 `vercel inspect`, 현재 로컬 `HEAD`보다 오래된 Preview 승격 차단, `vercel promote`, 운영 `/api/health` 확인 결과를 `reports/vercel-promote-latest.json`에 남긴다. 현재 최신 커밋 `f8b1b3cf`보다 먼저 만들어진 Preview만 남아 있어 실행 결과는 `blocked_stale_preview`다. GitHub/Vercel이 최신 커밋 Preview를 새로 만든 뒤 `npm run deploy:promote:latest`를 다시 실행한다.
