...
- 2026-06-13 KST 추가 확인: `deploy:promote:latest` 스크립트를 추가했다. 이 명령은 최신 `Ready` Preview를 자동 선택하고 `vercel inspect`, `vercel promote`, 운영 `/api/health` 확인 결과를 `reports/vercel-promote-latest.json`에 남긴다. 현재 실행 결과는 최신 Preview `https://halindosa-cwyibd0pv-titeks2-3861s-projects.vercel.app` 선택 성공, inspect 성공, promote는 Vercel Hobby 일일 제한(`api-deployments-free-per-day`)으로 `blocked_vercel_daily_limit`이다. 제한이 풀리면 `npm run deploy:promote:latest`만 다시 실행한다.
- GitHub CI: 최신 `main`/`codex/12h-product-ux-growth-hardening`에 `b0999052` push 완료. 새 커밋 후 운영 `/api/health`의 `deployment.shortCommit`으로 실제 반영 여부를 확인한다.
