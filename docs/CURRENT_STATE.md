...
- 2026-06-13 KST 최신 작업: 커밋 `ab5bc62b`은 공식 무료혜택 운영 feed를 Vercel env에 연결하기 위한 `.env.official-feeds.example`, `docs/OFFICIAL_FEED_ENV_ACTIVATION.md`, `reports/source-feed-env-template.json`을 추가했다. `source:activation:doctor`가 상위 공식 후보 24개뿐 아니라 lane별 env key 우선순위, first-party canary line, JSON/RSS/Atom/API feed 전용 금지 규칙을 생성하고, 관리자 `/admin` Feed Activation 패널도 env 템플릿과 금지 규칙을 보여준다. 확인 명령: `npm run source:activation:doctor`, `npm run lint`, `npm run smoke:local` 122/122, `npm run release:doctor` 194/194, `npm run build`, `npm run android:webview:doctor` 13/13, `npm run build:android`, `npm run cap:sync`, `npm run workspace:doctor:strict`. GitHub `main`/`codex/12h-product-ux-growth-hardening` push 완료. `npm run deploy:promote:latest`는 최신 커밋보다 오래된 Preview 승격을 `blocked_stale_preview`로 거부했다. 운영 `/api/health.deployment.shortCommit`은 아직 `baa49086`이다.
- 2026-06-13 KST 최신 작업: 커밋 `0cdb5928`은 공식 무료혜택 feed 활성화 후보 큐를 관리자/리포트/API 계약에 추가했다. `source:activation:doctor`는 starter pack 기반 상위 24개 공식 후보를 `topActivationCandidates`로 생성하고, 관리자 `/admin`과 `/api/admin/source-feed-activation`에서 우선 연결 후보를 확인할 수 있다. 확인 명령: `npm run source:activation:doctor`, `npm run lint`, `npm run smoke:local` 122/122, `npm run release:doctor` 194/194, `npm run build`, `npm run android:webview:doctor` 13/13, `npm run build:android`, `npm run cap:sync`, `npm run workspace:doctor:strict`. GitHub `main`/`codex/12h-product-ux-growth-hardening` push 완료. 최신 Preview `https://halindosa-2d2jv7p9g-titeks2-3861s-projects.vercel.app`는 Ready지만 Production promote는 Vercel Hobby 일일 제한(`api-deployments-free-per-day`)으로 `blocked_vercel_daily_limit` 상태다. 현재 운영 `/api/health.deployment.shortCommit`은 `baa49086`이며, 제한 해제 후 `npm run deploy:promote:latest`를 재실행하면 WebView 앱도 최신 운영 웹을 보게 된다.
- 2026-06-13 KST 최신 작업: 커밋 `fc7b0b49`는 CJ ONE 보너스포인트/기프트포인트, 해피포인트 제휴 혜택, 도미노피자 e-쿠폰/매니아 멤버십, LG전자 멤버십 가입 혜택, 이마트24 진행중인 이벤트/행사상품을 공식 무료혜택 소스 카탈로그에 추가했다. `source:catalog:report` 기준 공식 소스 후보 264개, `source:live:doctor` 기준 reachable/guarded 231/33, stale_or_removed 0건, `source:breadth:doctor`, `source:readiness:report`, `source:starter:pack`, `benefit:platform:report`, `lint`, `release:doctor` 194/194, `workspace:doctor:strict` 통과. GitHub `main`/`codex/12h-product-ux-growth-hardening` push 완료. 운영 `/api/health.deployment.shortCommit`은 `baa49086`까지 반영됐으며, `fc7b0b49` 직접 Production 배포는 Vercel Hobby 일일 제한(`api-deployments-free-per-day`)으로 실패했다. 제한 해제 후 `npx vercel deploy --prod --yes --scope titeks2-3861s-projects` 또는 최신 Ready Preview 생성 후 `npm run deploy:promote:latest`를 재시도한다.
- 2026-06-13 KST 최신 작업: 커밋 `baa49086`은 CJ ONE 공식 이벤트·쿠폰, CJ ONE 득템프, CU 공식 이달의 행사, CJ더마켓 공식 쿠폰&혜택을 공식 무료혜택 소스 카탈로그에 승격했다. `source:catalog:report` 기준 공식 소스 후보 256개, `source:live:doctor` 기준 reachable/guarded 224/32, stale_or_removed 0건, `source:breadth:doctor`, `source:readiness:report`, `source:starter:pack`, `benefit:platform:report`, `lint`, `release:doctor` 194/194, `build`, `workspace:doctor:strict` 통과. GitHub `main`/`codex/12h-product-ux-growth-hardening` push 완료. Vercel Production 직접 배포는 Hobby 일일 제한(`api-deployments-free-per-day`)으로 실패했으며, 운영 `/api/health.deployment.shortCommit`은 아직 `bd744c45`다. 제한 해제 후 `npx vercel deploy --prod --yes --scope titeks2-3861s-projects` 또는 최신 Ready Preview 생성 후 `npm run deploy:promote:latest`를 재시도한다.
...
- Vercel Production Deploy: `https://halindosa-8m85qcnvq-titeks2-3861s-projects.vercel.app`가 `www.halindosa.com`에 alias 되었고, 새 커밋 후에는 운영 `/api/health`의 `deployment.shortCommit`으로 최신 반영 여부를 다시 확인한다.
- 2026-06-13 KST 추가 확인: 커밋 `46ff7ad1`은 `benefit:platform:report`를 추가해 무료혜택 실시간 플랫폼 준비도를 `docs/FREE_BENEFIT_PLATFORM_READINESS.md`로 정리했다. 커밋 `84529c8f`은 이 리포트를 `qa`, `harness`, `release:doctor` 게이트에 연결했다. 확인 명령: `npm run benefit:platform:report`, `npm run qa:list`, `npm run lint`, `npm run release:doctor` 194/194, `npm run harness:ci`, `npm run workspace:doctor:strict`. GitHub `main`/`codex/12h-product-ux-growth-hardening` push 완료. 최신 Preview 승격은 Vercel Hobby 일일 제한(`api-deployments-free-per-day`) 때문에 Production 반영이 아직 막혀 있으며, 운영 `/api/health.deployment.shortCommit`은 `791cb102`다. 제한 해제 후 `npm run deploy:promote:latest`를 다시 실행한다.
- 2026-06-13 KST 추가 확인: 커밋 `791cb102`은 무료혜택 런타임 계약 게이트를 강화해 `id`, `imageUrl`, `sourceDomain`, `무료배송`, `브랜드이벤트`가 빠지면 `benefit:event:contract`와 smoke가 실패하도록 했다. `npm run benefit:event:contract`, `npm run benefit:model:doctor`, `npm run lint`, `npm run smoke:local` 119/119, `npm run release:doctor` 192/192, `npm run workspace:doctor:strict` 통과 후 GitHub `main`/`codex/12h-product-ux-growth-hardening` push 및 Vercel Production 직접 배포 완료. 운영 `/api/health`는 `officialBenefitVisibleCount=197`, `homepageVisibleRenderGuard=true`, `freeBenefitCollectionLaneOk=true`, `officialSourceFeedEnvActivationStatus=seed_fallback_only`를 반환한다.
...
- `/api/health`는 Vercel daily cron뿐 아니라 GitHub Actions `Benefit Refresh Scheduler`의 workflow 이름, 30분 무료혜택 갱신 주기, 60분 live feed 주기, `HALINDOSA_SITE_URL`, `CRON_SECRET`/`HALINDOSA_CRON_SECRET`, 수동 실행 명령까지 반환하도록 보강 중이다.
