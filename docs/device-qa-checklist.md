...
- 홈 `무료혜택 다음에 볼 상품` 가로 레일은 `옆으로 넘기기` 안내, 오른쪽 fade, 카드 단위 스냅이 실제 터치 스크롤에서도 자연스러운지 확인
...
- [ ] 홈 `무료혜택 다음에 볼 상품` 가로 레일이 옆으로 자연스럽게 넘어가고, 오른쪽 fade와 `옆으로 넘기기` 안내가 상품 CTA를 가리지 않는다.
