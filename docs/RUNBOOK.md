...
- 공식 혜택 feed 운영:
  ...
  - 운영자가 feed URL을 `.env`의 `DEAL_NEWS_FEED_URLS`, `DEAL_EVENT_NEWS_FEED_URLS`, `OFFICIAL_EVENT_FEED_URLS`, `PUBLIC_COUPON_FEED_URLS`에 넣은 뒤 `npm run news:feed:doctor && npm run news:feed:canary && npm run test:news-feed-errors && npm run test:news-feed-dry-run && npm run refresh:news && npm run verify:news && npm run news:freshness:doctor && npm run refresh:all`을 실행한다.
  ...
  - CSV의 `feed_transition` 행은 provider별 허용 소스, 필요한 환경변수, 현재 feed URL 수, 다음 액션을 포함한다. 공식 feed URL을 추가한 뒤에는 `npm run source:catalog:report && npm run source:live:doctor && npm run source:onboarding:plan && npm run source:feed-env:doctor && npm run source:readiness:report && npm run news:feed:doctor && npm run news:feed:canary && npm run refresh:all && npm run smoke:local && npm run release:doctor`를 순서대로 실행한다.
