...
- `npm run search:doctor`는 생활형 검색어 26개가 실제 상품 DB에 충분히 매칭되는지 검사한다. 상품 데이터를 늘린 뒤 이 명령이 실패하면 상품명/태그/카테고리 또는 alias를 함께 보강한다.
...
- 검색 alias를 추가할 때는 홈 추천 검색어에 노출할 가치가 있는지 함께 판단한다. 생수, 물티슈, 로켓, 지마켓, 배달쿠폰, 커피쿠폰, 라면, 햇반, 세제, 선크림, 유산균 같은 고의도 키워드는 `highIntentSearchKeywords`와 `search:doctor`, `smoke:local`을 같이 갱신한다.
...
- 홈/카테고리/찜 목록은 `QuickDealCard`를 사용한다. 이 카드는 모바일 상품 탐색용이므로 상품 이미지, 몰명, `가격 요약`, `구매 전 한눈에` 링크/시점/마감 요약, 배송, 찜, 공유, 구매하기를 먼저 보여주고, 신고/조건 상세는 상세 페이지와 신고 페이지에서 처리한다.
...
- `npm run search:doctor`는 라면, 햇반, 세제, 선크림, 유산균처럼 구매 의도가 뚜렷한 검색어가 넓은 카테고리 단어로 과도하게 확장되지 않고 제품명/브랜드/태그 중심으로 매칭되는지 검사한다.
