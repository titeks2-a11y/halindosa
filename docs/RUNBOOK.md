...
- 공식 혜택 feed 운영:
  ...
  - `npm run source:readiness:report`는 공식 소스 카탈로그, live 접근성, 온보딩 큐, feed env 안전성, 공식 혜택 노출, `refresh:all` 결과를 합쳐 `reports/source-readiness.json`과 `docs/SOURCE_READINESS_REPORT.md`를 생성한다. `/admin`의 `공식 소스 통합 준비도` 패널과 `GET /api/admin/source-readiness`에서 같은 내용을 확인하고, 검색 결과·커뮤니티 원문·HTML 랜딩 페이지가 운영 feed에 들어가지 않는지 출시 전에 한 번 더 점검한다.
