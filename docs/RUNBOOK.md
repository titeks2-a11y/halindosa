...
- 공식 혜택 feed 운영:
  ...
  - `/api/sources`의 `officialSourceCatalog` 요약은 공식 feed 후보 수, 고우선순위 후보, 카테고리/provider 커버리지, 다음 연결 env key를 반환한다. 공식 feed를 붙이기 전에는 이 값이 10개 필수 카테고리와 4개 provider를 모두 채우는지 먼저 본다.
