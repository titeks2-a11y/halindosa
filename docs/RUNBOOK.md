...
- `npm run search:doctor`는 생활형 검색어 35개가 실제 상품 DB에 충분히 매칭되는지 검사한다. 상품 데이터를 늘린 뒤 이 명령이 실패하면 상품명/태그/카테고리 또는 alias를 함께 보강한다.
...
- 검색 alias를 추가할 때는 홈 추천 검색어에 노출할 가치가 있는지 함께 판단한다. 생수, 물티슈, 계란, 우유, 닭가슴살, 마스크, 충전케이블, 화장지, 청소포, 김자반, 로켓, 지마켓, 배달쿠폰, 커피쿠폰, 라면, 햇반, 세제, 선크림, 유산균 같은 고의도 키워드는 `highIntentSearchKeywords`와 `search:doctor`, `smoke:local`을 같이 갱신한다.
