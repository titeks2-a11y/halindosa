...
- `npm run verify:links`는 `data/mockDeals.ts`의 전체 상품 ID와 `data/verifiedPurchaseLinks.ts`의 실제 구매 URL 매핑을 비교한다.
  ...
  - `npm run link:launch:gate`는 `reports/link-validation.json`, `reports/product-quality.json`, `reports/exposure-policy.json`, `reports/refresh-deals.json`, `reports/release-doctor.json`을 대조해 검색 링크, 품절/종료 링크, 깨진 링크, invalid URL 노출이 모두 0건인지 최종 확인한다. 결과는 `reports/link-launch-gate.json`과 `reports/LINK_LAUNCH_GATE.md`에 저장된다.
