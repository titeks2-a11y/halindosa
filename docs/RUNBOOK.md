...
- `npm run verify:links`는 `data/mockDeals.ts`의 전체 상품 ID와 `data/verifiedPurchaseLinks.ts`의 실제 구매 URL 매핑을 비교한다.
  ...
  - 라이브 검증 후에는 `npm run verify:products && npm run exposure:doctor && npm run link:launch:gate`를 다시 실행해 `reports/product-quality.json`, `reports/exposure-policy.json`, `reports/link-launch-gate.json`, `/api/admin/exposure-policy`에 live probe 요약, 실패 사유 분포, 상품별 감사 행, 최종 출시 판정이 함께 반영되도록 한다.
