...
- `npm run verify:links`는 `data/mockDeals.ts`의 전체 상품 ID와 `data/verifiedPurchaseLinks.ts`의 실제 구매 URL 매핑을 비교한다.
  ...
  - 관리자 화면의 `live probe 자동 본문 검증` 패널과 보호된 `GET /api/admin/live-probe-review`, `GET /api/admin/live-probe-review?format=csv`에서 같은 큐를 확인한다. CSV는 host별 우선 조치와 상품별 retry mode를 포함하므로, 운영자는 hard failure 0건을 먼저 확인한 뒤 접근보호/429 상품을 공식 API, 제휴 feed, 실기기 확인, backoff retry 순서로 보강한다.
