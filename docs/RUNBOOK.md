...
- `next.config.mjs`에는 기본 보안 헤더와 `output: "standalone"`이 설정되어 있습니다.
...
- 데이터 공급자가 실패하면 `DEAL_PROVIDER=mock`으로 fallback
...
- 네이버 쇼핑 공식 API를 쓰려면 `DEAL_PROVIDER=hybrid`, `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`을 설정합니다. 키가 없거나 API 장애가 있으면 mock fallback으로 화면을 유지합니다.
