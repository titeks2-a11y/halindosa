...
- 로컬 workspace 정리:
  ...
  - 같은 출력의 `Dirty regenerated report/data summary`와 `Dirty regenerated report/data files`는 QA/report/data 산출물이 현재 Git에서 얼마나 dirty인지 보여준다. summary는 루트 증빙, `docs` 증빙, `reports/`, `data` snapshot churn을 구분한다. 이 값이 높으면 `git add .`를 쓰지 말고 이번 변경과 직접 관련된 파일만 선별 stage한다.
  ...
  - `npm run clean:release-bundles:dry`는 `android/app/release`에 남은 업로드용 APK/AAB 복사본만 확인한다.
  - `npm run clean:release-bundles`는 Play Console 업로드가 끝난 release APK/AAB 복사본만 명시적으로 삭제한다.
  - `npm run clean:artifacts:deep`은 위 항목에 Capacitor plugin build cache까지 포함한다. `node_modules`, keystore, `.env`는 삭제하지 않는다.
- 공식 혜택 feed 운영:
  ...
  - 운영자가 feed URL을 `.env`의 `DEAL_NEWS_FEED_URLS`, `DEAL_EVENT_NEWS_FEED_URLS`, `OFFICIAL_EVENT_FEED_URLS`, `PUBLIC_COUPON_FEED_URLS`에 넣은 뒤에는 우선 `npm run news:feed:live`를 실행한다. 이 명령은 `source:feed-env:doctor`, `news:feed:doctor`, `news:feed:canary`, `refresh:news`, `verify:news`, `refresh:all`, `health:readiness`를 순서대로 실행하고 `reports/news-feed-live-pipeline.json`, `docs/NEWS_FEED_LIVE_PIPELINE.md`를 생성한다.
  - `news:feed:live`가 통과하면 설정 feed URL, canary 신선도, 공식 혜택 노출 수, 검색/비공식/종료 링크 0건, 전체 health readiness를 한 리포트에서 확인할 수 있다. 실패하면 리포트의 실패 step부터 고친 뒤 다시 실행한다.
  - GitHub Actions `CI`와 `Vercel Production Deploy` workflow는 같은 ref에 새 push가 들어오면 이전 실행을 자동 취소한다. 오래된 실패/취소 이력이 남아도 최신 커밋의 Vercel Production Deploy 성공과 최신 `CI` 실행을 기준으로 판단한다.
  - 개별 점검이 필요하면 `npm run news:feed:doctor && npm run news:feed:canary && npm run test:news-feed-errors && npm run test:news-feed-dry-run && npm run refresh:news && npm run verify:news && npm run news:freshness:doctor && npm run refresh:all`을 실행한다.
  - `npm run news:feed:canary`는 연결된 공식 feed URL을 짧게 가져와 HTTP 오류, 파싱 실패, 설정 feed 공백, 노출 후보 0건을 먼저 확인한다. 리포트는 `reports/news-feed-canary.json`과 `docs/NEWS_FEED_CANARY_REPORT.md`에 남고, `official feed canary` 상태는 `/api/health`, `/api/metrics`, 운영 헬스 리포트, `/admin`에서도 확인한다.
  - 운영 화면에서 같은 결과를 보려면 보호된 `GET /api/admin/news-feed-canary` 또는 `GET /api/admin/news-feed-canary?format=csv`를 사용한다. CSV는 `official_feed_canary`, 실패 provider, 다음 액션 행을 포함하므로 공식 feed 연결 직후 스프레드시트 점검에 바로 쓴다.
  ...
  - `/admin`의 `공식 feed preview` 패널과 보호된 `GET /api/admin/news-feed-preview`, `GET /api/admin/news-feed-preview?format=csv`에서 같은 preview 결과를 운영 화면과 스프레드시트로 확인한다. 검색 결과, 커뮤니티 원문, 블로그, 뉴스 기사 단독 링크가 `finalUrl`로 남아 있으면 운영 feed 환경변수에 반영하지 않는다.
  - `/admin`의 `공식 뉴스·혜택 feed 붙여넣기 검증` 패널은 운영자가 받은 JSON/RSS/CSV/NDJSON 본문을 바로 붙여넣어 POST `/api/admin/news-feed-preview`로 dry-run한다. 이 API는 임의 URL을 서버에서 fetch하지 않으며, 원문은 300KB 이하 단위로 나누어 검증한다. `visibleRows`만 운영 후보로 보고 `hiddenRows`는 공식 URL, 종료일, 제목, 혜택 조건 보강 후 다시 검증한다.
  - `npm run test:news-feed-dry-run`은 `reports/news-feed-dry-run-regression.json`을 만들고, RSS 본문 공식 링크 승격, CSV 파싱, NDJSON 중복 후보 숨김, 종료 문구 숨김, 검색 결과 URL 숨김, 뉴스 기사 단독 URL 숨김, 만료 공식 이벤트 숨김, POST dry-run route와 smoke negative case가 유지되는지 확인한다.
  ...
  - 환경변수로 연결한 공식 feed가 HTTP 오류, timeout, JSON/RSS/CSV/NDJSON 파싱 오류를 내면 `npm run verify:news`와 `npm run refresh:all`은 실패한다. seed fallback은 미연결 provider용 안전장치이며, 설정된 운영 feed 장애를 덮어 성공 처리하지 않는다. `npm run test:news-feed-errors`는 이 실패 게이트를 정상 feed/깨진 feed 양쪽으로 재현한다.
  ...
  - CSV의 `feed_transition` 행은 provider별 허용 소스, 필요한 환경변수, 현재 feed URL 수, 다음 액션을 포함한다. 공식 feed URL을 추가한 뒤에는 `npm run source:catalog:report && npm run source:live:doctor && npm run source:onboarding:plan && npm run source:readiness:report && npm run news:feed:live && npm run smoke:local && npm run release:doctor`를 순서대로 실행한다.
...
모바일 UX 규칙은 하단 safe-area padding, compact 검색창, 홈 검색 중복 방지, 카테고리 가로 칩, 쇼핑몰/가격/혜택 compact filter rail, compact 상품 카드, 하단 탭을 가리지 않는 토스트 위치, `무료혜택 다음에 볼 상품` 레일의 snap scroll, 오른쪽 fade, `옆으로 넘기기` 신호를 확인합니다. release doctor는 `test:mobile-ux`가 `qa`와 harness에 계속 포함되어 있고 `MOBILE_UX_REPORT.md`가 13개 게이트를 담는지 검사합니다.
...
- 홈 첫 화면의 `무료혜택 다음에 볼 상품` 레일은 무료혜택 히어로 아래에서 검증된 구매 링크 상품을 보조로 노출한다. 모바일에서는 snap scroll, 오른쪽 fade, `옆으로 넘기기` 신호가 함께 보여야 하고, 상품 DB를 늘린 뒤 이 레일이 비면 검색/필터 또는 링크 검증 기준이 과하게 좁아진 것이다.
...
- `npm run verify:links`는 `data/mockDeals.ts`의 전체 상품 ID와 `data/verifiedPurchaseLinks.ts`의 실제 구매 URL 매핑을 비교한다.
  ...
  - `reports/link-validation.json`은 `policy.source`, `httpStatusSummary`, 검색 링크 수, 품절/종료 신호 수, hidden 처리 수와 상품별 `auditedItems`를 기록한다. 각 감사 행은 `title`, `mallName`, `category`, `source`, `sourceName`, `originalUrl`, `finalUrl`, `affiliateUrl`, `eventUrl`, `linkType`, `availability`, `validationStatus`, `validationReason`, `validationCode`, `lastCheckedAt`, `priorityScore`, `isHidden`, `publishable`을 포함해야 한다. Play Store 제출 전에는 `launchGate.passed=true`, `actual.exposedSearchLinks=0`, `actual.exposedSoldOutLinks=0`, `actual.exposedBrokenLinks=0`, `actual.exposedInvalidUrls=0`, `actual.exposedNonPublishableItems=0`을 확인한다. 총 검증 대상 수보다 노출 수가 적어도 검색/품절/오류/불일치 링크를 숨긴 결과라면 정상이다.
  - `npm run qa`는 non-strict `npm run verify:links:live`를 포함해 Windows에서도 동일하게 redirect, 404, 410, 5xx, timeout, 접근 차단 신호를 `reports/link-validation.json`에 기록한다. `liveProbeReviewSummary.exposedHardFailureCount`, `hardFailureCount`, `transientNetworkCount`, `liveProbeReasonCounts`, `liveProbeHostFailureCounts`, `mismatchCategoryCounts`를 먼저 확인해 고객에게 노출되는 실패, 숨김 처리 후 보정이 필요한 실패, 일시 네트워크 신호, 쇼핑몰 접근 보호를 분리한다. `--strict`는 live probe 실패를 릴리즈 차단으로 보고, `--body`는 작은 응답 본문에서 품절/판매종료 문구까지 확인한다.
  ...
  - LF몰, 하이마트, 알리익스프레스처럼 클라이언트 렌더링 때문에 서버 HTML이 앱 셸처럼 보이는 상세 페이지는 `data/linkQualityPolicy.json.clientRenderedDetailHosts`에 등록된 호스트에서만 제한적으로 허용한다. 이 예외는 직접 상세 URL 패턴, `manual_review`/`partner_feed`/`official_api` 출처, 정상 HTTP 응답, 종료/품절 문구 없음, 검색/대표/커뮤니티 URL 아님을 모두 만족해야 하며 `reports/link-validation.json`의 `client_rendered_detail_manual_verified` evidence tier와 revalidation queue에 남는다.
  - 라이브 검증 후에는 `npm run verify:products && npm run exposure:doctor && npm run link:launch:gate`를 다시 실행해 `reports/product-quality.json`, `reports/exposure-policy.json`, `reports/link-launch-gate.json`, `/api/admin/exposure-policy`에 live probe 요약, 불일치 유형, 실패 사유 분포, 상품별 감사 행, 최종 출시 판정이 함께 반영되도록 한다. 출시 차단 기준은 고객 노출 실패인 `exposedHardFailureCount=0`이며, 숨김 항목의 `hardFailureCount`는 관리자 보정 큐로 다룬다.
  - `reports/product-quality.json`은 같은 정책의 `exposurePolicy`를 기록하고 `/api/deals` 노출 조건과 리포트 조건이 어긋나면 `verify:products`에서 실패한다.
  - `npm run exposure:doctor`는 `reports/exposure-policy.json`을 생성한다. 운영자는 여기서 `badExposedItems=0`, `searchLinksExposed=0`, `soldOutExposed=0`, `syntheticExposureScenarios.ok=true`를 확인한 뒤 신규 피드 반영 또는 스토어 제출을 진행한다. 합성 샘플은 정상 직접 구매 링크와 공식 혜택 링크는 통과시키고 검색 URL, 검색 linkType, 품절, 검증 실패, 운영 숨김, 위험 프로토콜, 대표몰 홈, 커뮤니티 원문, 최종 URL 누락 샘플은 모두 차단해야 한다.
  - `npm run link:launch:gate`는 `reports/link-validation.json`, `reports/product-quality.json`, `reports/exposure-policy.json`, `reports/refresh-deals.json`, `reports/release-doctor.json`을 대조해 검색 링크, 품절/종료 링크, 깨진 링크, invalid URL 노출이 모두 0건인지 최종 확인한다. 결과는 `reports/link-launch-gate.json`과 `reports/LINK_LAUNCH_GATE.md`에 저장된다. 운영자는 `/admin`의 `최종 링크 출시 게이트` 패널 또는 `/api/admin/link-launch-gate?format=csv`에서 Play Store 제출 직전 판정과 실패 노출 행을 내려받아 확인한다.
  - `npm run link:revalidation:report`는 `reports/link-validation.json`의 live probe 실패, 접근 보호, rate limit, request failure, 숨김 큐, 정기 순환 검수 후보를 `reports/link-revalidation-priority.json`과 `docs/LINK_REVALIDATION_PRIORITY.md`로 정리한다. 검색/품절/깨진 링크가 고객에게 노출되면 실패하고, 403/429처럼 쇼핑몰 접근 보호로 직접 확인이 막힌 상품은 숨기지 않되 공식 API, 제휴 피드, 실기기 확인 우선순위로 남긴다.
  - `npm run live:probe:review`는 non-strict live probe 결과 중 본문 확인이 막힌 상품을 `reports/live-probe-review.json`과 `docs/LIVE_PROBE_REVIEW_REPORT.md`로 다시 분류한다. 404/410/5xx/timeout/품절 문구 같은 hard failure는 릴리즈 차단이고, robots/403/429는 고객 노출 실패로 보지 않지만 official API, partner feed, manual device check, backoff retry 큐에 남겨 출시 전 우선 재확인한다. 접근 보호 큐는 7일 이내 `checkedAt` 수동 검수 증거가 있어야 하며, `manualEvidenceSummary.staleManualEvidenceCount`나 `missingManualEvidenceCount`가 1 이상이면 출시 전 재검수한다.
  - 관리자 화면의 `live probe 자동 본문 검증` 패널과 보호된 `GET /api/admin/live-probe-review`, `GET /api/admin/live-probe-review?format=csv`에서 같은 큐를 확인한다. CSV는 host별 우선 조치, 상품별 retry mode, `manualEvidenceStatus`, `manualEvidenceAgeDays`를 포함하므로, 운영자는 hard failure 0건과 stale/missing 수동 증거 0건을 먼저 확인한 뒤 접근보호/429 상품을 공식 API, 제휴 feed, 실기기 확인, backoff retry 순서로 보강한다.
...
- `clean:release-bundles`는 `android/app/release`의 APK/AAB 복사본만 지웁니다. Play Console 업로드 전에는 dry-run으로 위치만 확인합니다.
- `clean:artifacts:deep`은 Capacitor 플러그인이 `node_modules` 아래에 만든 Android build 캐시까지 지웁니다.
- keystore와 `.env*` 파일은 자동 삭제하지 않습니다.
