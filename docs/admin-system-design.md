...
운영자는 `needs_review`, `broken`, `sold_out` 상태의 상품을 링크 검수 큐에서 처리한다.

- 우선 검수: 핫딜, 마감임박, 높은 인기도, 낮은 구매 링크 신뢰도, 오류/품절 링크
- 보강 검수: 판매처 검색 fallback 상품, 중간 수준의 구매 링크 신뢰도
- 대기 검수: 노출 우선순위가 낮지만 상품 상세 URL 보강이 필요한 상품

큐에는 현재 이동 URL, 구매 링크 신뢰도, 링크 확인 시간, 보강 사유를 표시한다. 운영자는 커뮤니티 원문을 구매 링크로 쓰지 않고 실제 쇼핑몰 상품 상세 URL을 찾아 `finalPurchaseUrl`로 보강해야 한다.
