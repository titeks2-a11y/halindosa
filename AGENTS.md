...
- 기존 긴 대화는 `/exit`로 닫고, 새 작업은 새 `codex` 세션에서 시작한다.
- 이전 대화를 resume하지 않는다. 새 세션은 현재 워크트리, `docs/CURRENT_STATE.md`, 이 파일, 실제 명령 결과만 기준으로 판단한다.
...
- 커밋 시 `git add .`를 피하고 이번 작업 관련 파일만 명시적으로 stage한다.
