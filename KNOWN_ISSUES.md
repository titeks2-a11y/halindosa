...
- 상품 이미지 중 실상품 이미지가 아닌 카테고리 fallback이 많다. 현재 실상품 이미지 13개, fallback 127개이며 앱 화면은 깨지지 않는다. 홈 랭킹에는 실상품 이미지 가산점과 fallback 쏠림 완화가 들어갔지만, 운영 전환 시 실제 상품 이미지 공급률을 높여야 한다.
...
- Playwright 스크린샷 회귀 테스트는 아직 도입하지 않았다. 현재는 정적 UI rules, smoke:local, SEO/performance/link/image 하네스로 회귀를 막고 있다.
...
- Playwright 또는 Browser 기반 모바일 스크린샷 회귀 테스트를 별도 브랜치에서 도입한다.
