...
await check("home page", async () => {
  ...
  assert(text.includes("검수 완료 구매 링크"), "Home page missing verified purchase quick filter");
...
await check("deal link integrity", async () => {
  ...
  assert(
    verifiedDirectLinks.length >= 35 && verifiedDirectRate >= 65,
...
await check("verified direct purchase link coverage", async () => {
  ...
  assert(data.deals.length >= 35, `Expected at least 35 verified direct purchase deals, got ${data.deals.length}`);
  assert(
