...
await check("news deals api", async () => {
  ...
  assert(data.count >= 50, `Expected at least 50 official news/event benefits, got ${data.count}`);
  assert(
  ...
  assert(full.data.count >= 50, `News deals API should keep at least 50 visible official benefits, got ${full.data.count}`);
...
await check("admin news operations api", async () => {
  ...
  assert(data.report?.visibleCount >= 50, "Admin news operations report missing launch-ready visible official benefits");
  ...
  assert(data.report?.refreshAll?.newsDealsCount >= 50, "Admin news operations report missing refresh:all news count");
...
await check("admin news feed live pipeline api", async () => {
  ...
  assert(data.report?.officialBenefits?.visibleCount >= 50, "Admin news feed live pipeline should expose visible official benefits");
...
await check("admin source readiness rollup api", async () => {
  ...
  assert(data.report?.summary?.visibleOfficialBenefits >= 50, "Admin source readiness missing visible official benefits");
...
await check("admin daily operations api", async () => {
  ...
  assert(data.report?.summary?.visibleOfficialBenefits >= 50, "Admin daily operations should preserve official benefits");
...
await check("admin health readiness api", async () => {
  ...
  assert(data.report?.officialBenefits?.visibleCount >= 50, "Admin health readiness should preserve official benefit count");
  ...
  assert(data.report?.sourceReadiness?.visibleOfficialBenefits >= 50, "Admin health readiness should expose source readiness official benefits");
  ...
  assert(data.report?.cronRefresh?.newsDealsCount >= 50, "Admin health readiness should expose cron news count");
...
await check("cron refresh api guard", async () => {
  ...
  assert(data.refreshAll?.newsDealsCount >= 50, "Cron refresh dry-run missing latest refresh-all news count");
  ...
  assert(liveFeed.data.livePipeline?.officialBenefits?.visibleCount >= 50, "Cron liveFeed dry-run missing official benefits count");
...
await check("admin news revalidation priority api", async () => {
  ...
  assert(data.report?.summary?.visibleItems >= 50, "Official benefit revalidation priority should include visible official benefits");
  assert(data.report?.summary?.activeOfficialBenefits >= 50, "Official benefit revalidation priority should include active official benefits");
...
await check("admin official benefit alerts api", async () => {
  ...
  assert(data.report?.totals?.activeOfficialBenefits >= 50, "Official benefit alert admin report should expose active official benefits");
...
await check("health api", async () => {
  ...
  assert(data.checks?.officialBenefitVisibleCount >= 50, "Health API missing official benefit visible count");
  ...
  assert(data.checks?.cronRefreshLivePipelineOfficialBenefitsCount >= 50, "Health API missing cron live feed official benefit count");
  assert(data.checks?.cronRefreshProductDealsCount >= 140, "Health API missing cron refresh product count");
  assert(data.checks?.cronRefreshNewsDealsCount >= 50, "Health API missing cron refresh news count");
});
...
await check("official benefit alerts api", async () => {
  ...
  assert(data.recommendations?.summary?.totalActiveBenefits >= 50, "Official benefit alerts should use visible official benefit pool");
