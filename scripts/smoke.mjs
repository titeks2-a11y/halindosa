...
await check("health api", async () => {
  ...
  assert(data.checks?.firstPartyFreeBenefitFeedOk === true, "Health API missing passing first-party free benefit feed status");
  assert(data.checks?.firstPartyFreeBenefitFeedEndpoint === "/api/feeds/free-benefits", "Health API missing first-party free benefit feed endpoint");
  assert(data.checks?.firstPartyFreeBenefitFeedSource === "data/refreshedNewsDeals.json", "Health API missing first-party free benefit feed source");
  assert(data.checks?.firstPartyFreeBenefitFeedPublishableCount >= 100, "Health API missing first-party publishable free benefit count");
  assert(data.checks?.firstPartyFreeBenefitFeedConsumerCount >= 80, "Health API missing consumer-first first-party free benefit count");
  assert(data.checks?.firstPartyFreeBenefitFeedSearchLinkCount === 0, "Health API found first-party free benefit search links");
  assert(data.checks?.firstPartyFreeBenefitFeedHomepageLikeCount === 0, "Health API found first-party homepage-like benefit links");
  assert(data.checks?.firstPartyFreeBenefitFeedDuplicateGroups === 0, "Health API found first-party duplicate benefit groups");
