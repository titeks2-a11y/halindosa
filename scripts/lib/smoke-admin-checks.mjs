...
export async function runAdminDashboardSmokeChecks() {
  await check("admin dashboard quality cards", async () => {
    ...
    assert(
      text.includes("무료혜택 카테고리 커버리지") &&
        text.includes("전원증정·선착순·쿠폰·샘플 공백") &&
    ...
    assert(text.includes("NDJSON") && text.includes("CSV"), "Admin dashboard missing CSV/NDJSON source feed format policy");
    ...
    assert(
      text.includes("무료혜택 수집축 운영 리포트") &&
        text.includes("공식 이벤트·쿠폰·샘플 수집 공백") &&
      ...
      "Admin dashboard missing free benefit collection lanes operation panel"
    );
