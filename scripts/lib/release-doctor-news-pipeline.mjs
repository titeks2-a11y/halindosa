...
export function checkNewsDealPipeline() {
  ...
  if (
    ...
    Number(feedLivePipelineReport.officialBenefits?.visibleCount ?? 0) < 50 ||
  ...
  if (existsSync(join(root, "reports/news-deals.json"))) {
    ...
    if ((report.visibleCount ?? 0) < 50) issues.push("news-deals report should include at least 50 visible official benefits across daily benefit categories");
  ...
  if (existsSync(join(root, "reports/news-freshness.json"))) {
    ...
    if ((report.visibleCount ?? 0) < 50) issues.push("news-freshness report should include at least 50 visible official benefits");
  ...
  if (existsSync(join(root, "reports/news-revalidation-priority.json"))) {
    ...
    if ((report.summary?.visibleItems ?? 0) < 50) issues.push("news-revalidation-priority report should include at least 50 visible official benefits");
    if ((report.summary?.activeOfficialBenefits ?? 0) < 50) issues.push("news-revalidation-priority report should include at least 50 active official benefits");
  ...
  if (existsSync(join(root, "reports/refresh-all.json"))) {
    ...
    if ((report.newsDealsCount ?? 0) < 50) issues.push("refresh-all should include expanded official news/event benefits");
