...
export function checkNewsDealPipeline() {
  ...
  if (
    ...
    Number(feedLivePipelineReport.officialBenefits?.visibleCount ?? 0) < MIN_OFFICIAL_BENEFITS ||
  ...
  if (existsSync(join(root, "reports/news-deals.json"))) {
    ...
    if ((report.visibleCount ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push(`news-deals report should include at least ${MIN_OFFICIAL_BENEFITS} visible official benefits across daily benefit categories`);
  ...
  if (existsSync(join(root, "reports/news-freshness.json"))) {
    ...
    if ((report.visibleCount ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push(`news-freshness report should include at least ${MIN_OFFICIAL_BENEFITS} visible official benefits`);
  ...
  if (existsSync(join(root, "reports/events-refresh.json"))) {
    ...
    if ((report.visibleCount ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push(`events-refresh should expose at least ${MIN_OFFICIAL_BENEFITS} verified official event/coupon benefits`);
    if ((report.exposedSearchLinks ?? 999) !== 0 || (report.exposedNonOfficialLinks ?? 999) !== 0) {
  ...
  if (existsSync(join(root, "reports/news-revalidation-priority.json"))) {
    ...
    if ((report.summary?.visibleItems ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push(`news-revalidation-priority report should include at least ${MIN_OFFICIAL_BENEFITS} visible official benefits`);
    if ((report.summary?.activeOfficialBenefits ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push(`news-revalidation-priority report should include at least ${MIN_OFFICIAL_BENEFITS} active official benefits`);
  ...
  if (existsSync(join(root, "reports/refresh-all.json"))) {
    ...
    if ((report.newsDealsCount ?? 0) < MIN_OFFICIAL_BENEFITS) issues.push("refresh-all should include expanded official news/event benefits");
