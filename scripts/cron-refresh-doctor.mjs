...
const checks = [
  ...
  githubBenefitRefreshWorkflow.includes('cron: "*/30 * * * *"') &&
    githubBenefitRefreshWorkflow.includes("force_live_feed") &&
    githubBenefitRefreshWorkflow.includes("minimum_visible_benefits") &&
    githubBenefitRefreshWorkflow.includes("FORCE_LIVE_FEED") &&
    githubBenefitRefreshWorkflow.includes("MINIMUM_VISIBLE_BENEFITS") &&
    ...
    ? pass("github scheduled benefit refresh", "GitHub Actions can call protected benefits refresh every 30 minutes, live feed refresh hourly or on manual force, and verify production /api/health plus /api/freebies after refresh.")
    : fail("github scheduled benefit refresh", "A protected GitHub Actions scheduler should call /api/cron/benefits frequently, support manual force_live_feed and minimum_visible_benefits inputs, call /api/cron/refresh?mode=liveFeed on a bounded cadence, then verify production /api/health and /api/freebies readiness."),
...
- 더 빠른 무료혜택 갱신은 GitHub Actions \`Benefit Refresh Scheduler\`가 \`CRON_SECRET\` 또는 \`HALINDOSA_CRON_SECRET\`이 있을 때 30분마다 \`/api/cron/benefits\`를 호출하고, 정각 또는 수동 \`force_live_feed=true\` 실행 때 \`/api/cron/refresh?mode=liveFeed\`도 호출합니다.
