...
- 홈/카테고리/찜 목록은 `QuickDealCard`로 상품 이미지, 현재가, 정상가, 구매 전 한눈에 요약, 배송, 찜, 공유, 구매하기를 압축 표시
...
상품 카드의 `구매 전 한눈에` 요약은 링크 확인 상태, 가격 확인 시점, 마감 시간을 세 칸으로 압축합니다. 사용자는 상세 페이지를 열기 전에 목록에서 바로 안전하게 이동할 상품인지 먼저 판단할 수 있습니다.
