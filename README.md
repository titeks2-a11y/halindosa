...
- 생수, 물티슈, 계란, 우유, 닭가슴살, 마스크, 충전케이블처럼 자주 쓰는 생활형 검색어를 추천 검색어로 우선 노출
...
생활형 검색어는 동의어로 확장합니다. 예를 들어 `생필품`은 생활용품/생활필수/물티슈/세제/생수 계열을 함께 찾고, `무배`는 무료배송/로켓배송/네멤무료 표현을 함께 찾습니다. 라면, 햇반, 계란, 우유, 닭가슴살, 마스크, 충전케이블처럼 구매 의도가 뚜렷한 키워드는 넓은 카테고리보다 제품명/브랜드/태그 중심으로 매칭합니다. 이 동의어 검색은 홈과 API가 같은 기준으로 적용하며 `npm run smoke:local`에서 재검증합니다.
...
추천 검색어에는 `생수`, `물티슈`, `계란`, `우유`, `닭가슴살`, `마스크`, `충전케이블`, `멀티탭`, `로켓`, `지마켓`, `배달쿠폰`, `커피쿠폰`, `라면`, `햇반`, `세제`, `유산균`처럼 앱 사용자가 바로 눌러볼 만한 생활형 검색어를 먼저 섞습니다. 이 키워드는 `data/searchAliases.json`과 `npm run search:doctor` 기준으로 관리합니다.
...
`search:doctor`는 `생필품`, `무배`, `0원`, `가전제품`, `편의점`, `앱테크`, `육아템`, `로켓`, `지마켓`, `배달쿠폰`, `치킨쿠폰`, `커피쿠폰`, `무료커피`, `영화무료`, `패션`, `우산`, `생수`, `물티슈`, `기저귀`, `치약`, `라면`, `햇반`, `세제`, `선크림`, `유산균`, `계란`, `우유`, `닭가슴살`, `마스크`, `충전케이블`, `멀티탭` 같은 짧은 생활형 검색어가 실제 상품 DB에 충분히 연결되는지 확인합니다.
