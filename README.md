...
- 홈 첫 화면의 `무료혜택 다음에 볼 상품` 가로 상품 레일로 무료혜택 확인 뒤 비교할 보조 할인 상품을 노출
...
npm run clean:artifacts:mobile:dry
...
- `workspace:doctor`: 추적 중인 QA/report/data 산출물이 dirty 상태로 얼마나 남아 있는지도 함께 보여주므로, `git add .` 전에 이번 작업과 무관한 리포트가 섞였는지 확인
...
npm run surface:publishable:doctor
...
- `npm run test:mobile-ux`는 홈 검색창 중복, 카테고리/필터 가로 칩, 하단 탭 safe-area, compact 카드, 토스트 위치와 `무료혜택 다음에 볼 상품` 레일의 스냅, 오른쪽 fade, `옆으로 넘기기` 신호를 13개 게이트로 검사하고 `MOBILE_UX_REPORT.md`를 갱신합니다.
