...
- 로켓, 지마켓, 배달쿠폰, 커피쿠폰, 영화무료처럼 자주 쓰는 생활형 검색어를 추천 검색어로 우선 노출
...
추천 검색어에는 `로켓`, `지마켓`, `배달쿠폰`, `커피쿠폰`, `영화무료`, `생수`, `물티슈`, `기저귀`처럼 앱 사용자가 바로 눌러볼 만한 생활형 검색어를 먼저 섞습니다. 이 키워드는 `data/searchAliases.json`과 `npm run search:doctor` 기준으로 관리합니다.
...
`search:doctor`는 `생필품`, `무배`, `0원`, `가전제품`, `편의점`, `앱테크`, `육아템`, `로켓`, `지마켓`, `배달쿠폰`, `커피쿠폰`, `영화무료`, `생수`, `물티슈`, `기저귀`, `치약` 같은 짧은 생활형 검색어가 실제 상품 DB에 충분히 연결되는지 확인합니다.
