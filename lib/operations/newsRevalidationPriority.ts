...
export function buildNewsRevalidationPriorityCsv(report: NewsRevalidationPriorityReport) {
  const summaryRows = [
    ["total_items", "전체 공식 혜택", report.summary.totalItems, report.summary.totalItems >= 60 ? "pass" : "review", "검증 대상 공식 혜택", "refresh:news 재실행"],
    ["visible_items", "노출 공식 혜택", report.summary.visibleItems, report.summary.visibleItems >= 60 ? "pass" : "review", "사용자 공개 후보", "카테고리 커버리지 확인"],
    ["active_official_benefits", "활성 공식 혜택", report.summary.activeOfficialBenefits, report.summary.activeOfficialBenefits >= 60 ? "pass" : "review", "알림/추천 후보", "official:alerts:report 재실행"],
