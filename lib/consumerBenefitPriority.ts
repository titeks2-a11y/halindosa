...
const consumerBenefitPattern = /쿠폰|무료\s*샘플|샘플|기프티콘|교환권|전원\s*증정|선착순|신규\s*가입|웰컴|출석|룰렛|포인트|캐시백|무료배송|무배|1\+1|2\+1|편의점|마트|배달|카페|뷰티|올리브영|무신사|컬리|SSG|롯데온|롯데잇츠|H\.?Point|해피포인트|CJ\s*ONE|다이소|G마켓|11번가|쿠팡|이마트|홈플러스|브랜드/i;
...
export function getConsumerBenefitPriorityAdjustment(deal: NewsDeal) {
  ...
  if (/무료|0원|전원\s*증정|샘플|기프티콘|쿠폰|신규\s*가입|웰컴/.test(text)) score += 14;
  if (/공식|멤버십|브랜드|앱\s*전용/.test(text) && /쿠폰|포인트|샘플|혜택/.test(text)) score += 8;
