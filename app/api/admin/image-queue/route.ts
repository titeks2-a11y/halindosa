  ...
  if (format === "csv") {
    const csv = toCsv(
      imageQuality.priorityDeals.map((deal, index) => ({
        ...
        imagePolicyKey: deal.imagePolicyKey,
        imageAcquisitionChannel: deal.imageAcquisitionChannel,
        recommendedImageSource: deal.recommendedImageSource,
        imageFeedFields: deal.imageFeedFields?.join("|") ?? "",
        requiredFeedFields: deal.requiredFeedFields?.join("|") ?? "",
        sourceSafetyLevel: deal.sourceSafetyLevel,
        imageReadyGate: deal.imageReadyGate,
        operatorChecklist: deal.operatorChecklist?.join("|") ?? "",
        requestTemplate: deal.requestTemplate,
        imageRightsChecklist: deal.imageRightsChecklist?.join("|") ?? "",
        imageManualVerification: deal.imageManualVerification,
        prohibitedImageSource: deal.prohibitedImageSource,
        ...
        operatingTargetRate: imageQuality.sourcingPlan.operatingTargetRate,
        gapToLaunchTarget: imageQuality.sourcingPlan.gapToLaunchTarget,
        gapToOperatingTarget: imageQuality.sourcingPlan.gapToOperatingTarget,
        weeklyOperatingBatchSize: imageQuality.sourcingPlan.weeklyOperatingBatchSize,
