...
function buildNewsOperationsCsv(report: NewsOperationsReport) {
  ...
  report.providerStats.forEach((provider) => {
    rows.push([
      ...
      provider.configuredEmptyFeed ? "configured_empty_feed" : provider.configured ? "external_feed_configured" : "seed_fallback",
      `seed=${provider.seedCount ?? 0};feed=${provider.feedItemCount ?? 0};success=${provider.feedSuccessCount ?? 0}/${provider.feedUrls ?? 0};empty=${Boolean(provider.configuredEmptyFeed)}`,
