    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <section className="rounded-3xl border border-sky-100 bg-white p-5 shadow-sm" aria-label="최종 링크 출시 게이트">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            {[
              ["감사 상품", linkLaunchGate.actual.auditedItems, "전체 제출 후보"],
              ["노출 상품", linkLaunchGate.actual.exposedItems, "사용자 공개"],
              ["검증 구매 링크", linkLaunchGate.actual.verifiedPurchaseLinks, "구매/공식 혜택"],
              ["검색 링크 노출", linkLaunchGate.actual.exposedSearchLinks, "항상 0"],
              ["품절/종료 노출", linkLaunchGate.actual.exposedSoldOutLinks, "항상 0"],
              ["실패 노출", linkLaunchGate.actual.failedExposureItems, "항상 0"]
            ].map(([label, value, description]) => (
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[0.8fr_1.2fr]">
            <div className={`rounded-2xl border p-4 ${linkLaunchGate.ok ? "border-emerald-100 bg-emerald-50" : "border-red-100 bg-red-50"}`}>
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-black text-slate-950">Play Store 제출 판정</p>
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${linkLaunchGate.ok ? "bg-white text-emerald-700" : "bg-white text-dossa-red"}`}>
                  {linkLaunchGate.ok ? "통과" : "차단"}
                </span>
              </div>
              <p className="mt-2 text-xs font-bold leading-5 text-slate-600">
                생성 시각 {linkLaunchGate.generatedAt ? getRelativeTime(linkLaunchGate.generatedAt) : "리포트 없음"} · release doctor 실패 {linkLaunchGate.actual.releaseDoctorFailures}개
              </p>
              <p className="mt-2 text-xs font-bold leading-5 text-slate-600">
                깨진 링크 {linkLaunchGate.actual.exposedBrokenLinks}개 · invalid URL {linkLaunchGate.actual.exposedInvalidUrls}개 · hidden {linkLaunchGate.actual.hiddenProducts}개
              </p>
              <p className="mt-2 text-xs font-bold leading-5 text-slate-600">
                live hard failure {linkLaunchGate.actual.liveHardFailures}개 · 품절 본문 신호 {linkLaunchGate.actual.sellerUnavailableSignals}개
              </p>
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-slate-700 shadow-sm">
                  availability={linkLaunchGate.policy.exposurePolicy?.availability ?? "active"}
                </span>
                <span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-slate-700 shadow-sm">
                  validation={linkLaunchGate.policy.exposurePolicy?.validationStatus ?? "passed"}
                </span>
                <span className="rounded-full bg-white px-3 py-1.5 text-xs font-black text-slate-700 shadow-sm">
                  검색·대표몰·품절 링크 0건
                </span>
          ...
          <div className="mt-4 rounded-2xl border border-sky-100 bg-sky-50 p-4">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-sky-700 shadow-sm">
                {linkLaunchGate.liveProbeReviewSummary.status ?? "unknown"}
              </span>
            </div>
            <p className="mt-2 text-xs font-bold leading-5 text-sky-900/75">
              {linkLaunchGate.liveProbeReviewSummary.interpretation ?? "라이브 검증 해석 없음"}
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {(linkLaunchGate.issues.length ? linkLaunchGate.issues : ["이슈 없음"]).map((issue) => (
